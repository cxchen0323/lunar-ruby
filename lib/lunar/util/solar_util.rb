...
module Lunar
  module Util
    class SolarUtil
      # 星期
      WEEK = ["日", "一", "二", "三", "四", "五", "六"].freeze

      # 每月天数
      DAYS_OF_MONTH = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31].freeze

      # 星座
      XING_ZUO = ["白羊", "金牛", "双子", "巨蟹", "狮子", "处女", "天秤", "天蝎", "射手", "摩羯", "水瓶", "双鱼"].freeze
      ...
      def self.getDaysOfMonth(year, month)
        # 获取某年某月有多少天
        # :param year: 年
        # :param month: 月
        # :return: 天数
      ...
      def self.getDaysInYear(year, month, day)
        days = 0
        (1...month).each do |i|
      ...
      def self.getWeeksOfMonth(year, month, start)
        # 获取某年某月有多少周
        # :param year: 年
        # :param month: 月
        ...
        ((getDaysOfMonth(year, month) + Lunar::Solar.fromYmd(year, month, 1).getWeek - start) * 1.0 / WEEK.length).ceil
      end

      def self.getDaysBetween(ay, am, ad, by, bm, bd)
        if ay == by
